...
/**
 * REST service interface for mongo panache query actions.
 *
 * @author balazs.joo
 * @since 0.1.0
 */
@Tag(name = "REST endpoints for mongo panache service calls", description = "REST endpoints for mongo panache service calls")
@Path(MongoPanacheServicePath.REST_MONGO_PANACHE_SERVICE_QUERY)
public interface IMongoPanacheQueryRest {
    ...
    @GET
    @Path("/byStatus")
    @Operation(summary = "Get all by status", description = "Retrieves all mongo sample entities with the specified status.")
    @Produces(MediaType.APPLICATION_JSON)
    BaseResponse getAllByStatus(@QueryParam("status") SampleStatusEnumType status);

    @GET
    @Path("/between")
    @Operation(summary = "Get all between", description = "Retrieves all mongo sample entities between the specified dates.")
    @Produces(MediaType.APPLICATION_JSON)
    BaseResponse getAllBetween(@QueryParam("from") String fromString, @QueryParam("to") String toString) throws BaseException;

    @GET
    @Path("/idsBetween")
    @Operation(summary = "Get all ids between", description = "Retrieves the IDs of all mongo sample entities between the specified dates.")
    @Produces(MediaType.APPLICATION_JSON)
