...
/**
 * REST service interface for panache projection actions.
 *
 * @author balazs.joo
 * @since 0.1.0
 */
@Tag(name = "REST endpoints for panache service calls", description = "REST endpoints for panache service calls")
@Path(PanacheServicePath.REST_PANACHE_SERVICE_PROJECTION)
public interface IPanacheProjectionRest {

    @Operation(summary = "Get all between with projection",
            description = "Retrieves all sample entities between the specified dates using projection.")
    @GET
    @Path("/between")
    ...
    @Operation(summary = "Get all by status with projection",
            description = "Retrieves all sample entities with the specified status using projection.")
    @PUT
    @Path("/byStatus")
    @Produces(MediaType.APPLICATION_JSON)
