...
/**
 * Mongo Panache service {@link IMongoPanacheProjectionRest} tests
 *
 * @author balazs.joo
 * @since 0.1.0
 */
@DisplayName("Testing Mongo Panache service - Projection")
@Tag(TestSuiteGroup.JAXRS)
class MongoPanacheProjectionRestIT extends BaseSampleIT {
    ...
    @Test
    @DisplayName("Testing getAllByValueProjection()")
    void testGetAllByValueProjection() {
        BaseResponse response = getRestClient(IMongoPanacheProjectionRest.class).getAllByStatusProjection(SampleStatusEnumType.DONE);
    ...
    void testGetAllBetweenWithProjection() throws BaseException {
        ...
        BaseResponse response = getRestClient(IMongoPanacheProjectionRest.class).getAllBetweenWithProjection(from, to);
        Assertions.assertNotNull(response);
        Assertions.assertEquals(FunctionCodeType.OK, response.getFuncCode());
    }
